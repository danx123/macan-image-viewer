...
# --- PENAMBAHAN: Library baru untuk membuka URL web dan partial functions ---
import webbrowser
from functools import partial

# --- Mengimpor library baru yang dibutuhkan ---
...
from PyQt6.QtWidgets import (
    ...
    # --- PENAMBAHAN: Dialog dan RadioButton untuk UI Resize ---
...
from PyQt6.QtCore import (
    ...
    # --- PENAMBAHAN: Library QSettings untuk menyimpan state ---
    QSettings
)
from PyQt6.QtSvg import QSvgRenderer
from PyQt6.QtPrintSupport import QPrintDialog, QPrinter

# --- Teks untuk Image Converter Widget ---
CONVERTER_TEXT = {
...
# --- Kelas Worker untuk Konversi Gambar ---
...
# --- Widget Baru untuk Image Converter ---
...
# --- PENAMBAHAN: Kelas Dialog untuk fitur Resize Image ---
class ResizeDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # --- PERBAIKAN: Masukkan widget piksel ke dalam container QWidget ---
        ...
        # Container untuk widget persentase
    ...
    # --- PERBAIKAN: Logika untuk beralih antara mode piksel dan persentase ---
    def on_mode_change(self, checked):
        ...
        if not is_pixels_mode:
            self.percentage_slider.setValue(100) # Reset saat beralih ke percentage
    ...
    def update_new_labels(self):
        ...
        self.current_label.setText(f"Current: {self.original_width} x {self.original_height} pixels   {filesize_str}   {os.path.splitext(self.parent().image_path)[1].upper()[1:]}")
        
        new_w = self.width_edit.text()
        new_h = self.height_edit.text()
        file_type = self.file_type_combo.currentText()
        # Note: File size estimation is complex. We'll show "Est." and not calculate it.
...
class ImageViewer(QMainWindow):
    ...
    Resize Dialog, Filmstrip, Save State.
    """
    def __init__(self):
        super().__init__()
        
        # --- PENAMBAHAN: Inisialisasi QSettings ---
        ...
        # --- PENAMBAHAN: Memuat pengaturan terakhir ---
        self.load_settings()

    def init_ui(self):
        self.setWindowTitle('Macan Image Viewer')
        self.setGeometry(100, 100, 1100, 650)
        # Hapus center_window() agar posisi dari QSettings yang digunakan
        # self.center_window() 
        ...
        # --- PENAMBAHAN: Membuat widget filmstrip ---
        ...
        # --- PENAMBAHAN: Mengaktifkan context menu policy ---
    ...
    def _create_actions(self):
        ...
        # --- PENAMBAHAN: Aksi baru untuk context menu ---
        ...
        self.filmstrip_action.setChecked(True) # Tampil secara default
    ...
    # --- PENAMBAHAN: Metode untuk membuat UI filmstrip ---
    def _create_filmstrip(self):
        ...
        self.filmstrip_scroll_area.hide() # Sembunyikan by default, diatur oleh action

    # --- PENAMBAHAN: Metode untuk mengisi filmstrip dengan thumbnail ---
    def _populate_filmstrip(self):
        ...
        for index, image_path in enumerate(self.current_folder_images):
            ...
            thumbnail_button.setToolTip(os.path.basename(image_path))
            ...
            thumbnail_button.clicked.connect(partial(self.open_image, image_path))
            self.filmstrip_layout.addWidget(thumbnail_button)
            
        # Spacer di akhir untuk layout yang lebih baik
    ...
    def _update_action_states(self, enabled):
        if self.slideshow_running:
            actions_to_disable = [
                self.open_action, self.save_as_action, self.copy_action,
                self.open_explorer_action, self.zoom_in_action, self.zoom_out_action,
                ...
                self.save_pdf_action, self.set_wallpaper_action, self.prev_action,
                self.next_action, self.undo_action, self.redo_action, self.crop_action,
                self.resize_action # --- PENAMBAHAN: Disable resize saat slideshow ---
            ]
        ...
        actions = [
            ...
            self.set_wallpaper_action, self.fullscreen_action, self.crop_action,
            # --- PENAMBAHAN: Update state aksi baru ---
            self.file_info_action, self.resize_action, self.visual_search_action
        ]
        ...
        self.slideshow_action.setEnabled(enabled and len(self.current_folder_images) > 1)
        # --- PENAMBAHAN: Atur state untuk filmstrip action ---
        self.filmstrip_action.setEnabled(enabled)
        
    def open_image(self, file_path=None):
        if file_path and self.image_path == file_path: return # Jangan buka ulang file yang sama

        if not file_path:
            self.stop_slideshow()
            # --- MODIFIKASI: Gunakan direktori terakhir dari QSettings ---
            file_path, _ = QFileDialog.getOpenFileName(self, "Open Image", self.last_directory, "Image Files (*.png *.jpg *.jpeg *.bmp *.webp);;All Files (*)")
        
        if file_path:
            # --- PENAMBAHAN: Simpan direktori yang baru digunakan ---
            ...
            if self.cv_image is None:
                self.statusbar.showMessage(f"Error: Failed to open file {os.path.basename(self.image_path)}", 5000)
                return
            ...
            is_new_folder = not self.current_folder_images or os.path.dirname(file_path) != os.path.dirname(self.current_folder_images[0])
            if is_new_folder:
                ...
                # Cukup update highlight di filmstrip tanpa load ulang semua thumbnail
                for i in range(self.filmstrip_layout.count() -1): # -1 untuk spacer
    ...
    def show_about_dialog(self):
        ...
        msg_box.setText("<b>Macan Image Viewer v2.6.0</b><br><br>" 
    ...
    def dragEnterEvent(self, event):
        if event.mimeData().hasUrls():
            ...
            if any(any(url.toLocalFile().lower().endswith(ext) for ext in supported_extensions) for url in urls):
    ...
    def is_pannable(self):
        return self.scroll_area.horizontalScrollBar().isVisible() or \
    ...
    def image_mouse_press(self, event):
        ...
        # --- MODIFIKASI: Menangani klik kanan untuk menu konteks ---
        # Logika klik kanan kini ditangani oleh show_context_menu,
    ...
    def start_slideshow(self):
        if len(self.current_folder_images) < 2:
            self.statusbar.showMessage("Not enough images in the folder for a slideshow.", 3000)
            return
    ...
    # --- PENAMBAHAN: Metode baru untuk Context Menu ---
    ...
    def show_file_info(self):
        if self.image_path and os.path.exists(self.image_path) and self.cv_image is not None:
            ...
            info_text = (
                f"<b>Filename:</b> {os.path.basename(self.image_path)}<br>"
                f"<b>Path:</b> {os.path.dirname(self.image_path)}<br>"
                f"<b>Dimensions:</b> {w} x {h} pixels<br>"
                f"<b>File Size:</b> {filesize_str} ({size_bytes:,} bytes)"
            )
            # --- PERBAIKAN: Tambahkan stylesheet agar teks info terlihat ---
            msg_box = QMessageBox(self)
            msg_box.setWindowTitle("File Info")
            msg_box.setTextFormat(Qt.TextFormat.RichText)
            msg_box.setText(info_text)
            msg_box.setStyleSheet("QLabel { color: #000000; }") # Atur warna teks menjadi hitam
            msg_box.exec()


    def resize_image(self):
        if self.display_image is None or not self.image_path: return
        ...
        if dialog.exec():
            ...
            try:
                # Lakukan resize pada gambar yang sedang ditampilkan
                resized_image = cv2.resize(self.display_image, (new_width, new_height), interpolation=cv2.INTER_AREA)

                # Buka dialog Save As untuk menyimpan gambar hasil resize
                default_filename = f"{os.path.splitext(os.path.basename(self.image_path))[0]}_resized.{values['format']}"
                ...
                if save_path:
                    ...
                    # Tentukan opsi penyimpanan berdasarkan kualitas
                    params = []
    ...
    def visual_search(self):
        if self.image_path:
            # Google Lens tidak mendukung upload file lokal via URL. 
            # Cara terbaik adalah membuka halaman uploadnya.
            url = "https://lens.google.com/"
            webbrowser.open(url)
            self.statusbar.showMessage("Opening Google Lens. Please upload your image.", 4000)
    ...
    def save_image_as(self):
        if self.display_image is None: return
        default_path = os.path.join(self.last_directory, os.path.basename(self.image_path) if self.image_path else "")
        # --- MODIFIKASI: Gunakan direktori terakhir dari QSettings ---
    ...
    # --- PENAMBAHAN: Metode untuk memuat dan menyimpan pengaturan ---
    def load_settings(self):
        """Memuat pengaturan jendela, direktori, dan filmstrip."""
        geometry = self.settings.value("geometry", QByteArray())
        if geometry.size() > 0:
            self.restoreGeometry(geometry)
        else:
            # Jika tidak ada state, tengahkan jendela saat pertama kali dibuka
            self.center_window()
        ...
        # Panggil toggle_filmstrip setelah UI siap untuk menghindari error
        # self.toggle_filmstrip(filmstrip_visible) 
        
    def save_settings(self):
        """Menyimpan pengaturan jendela, direktori, dan filmstrip."""
        self.settings.setValue("geometry", self.saveGeometry())
        self.settings.setValue("last_directory", self.last_directory)
        self.settings.setValue("filmstrip_visible", self.filmstrip_action.isChecked())
             
    def closeEvent(self, event):
        # --- PENAMBAHAN: Simpan pengaturan sebelum keluar ---
        self.save_settings()
