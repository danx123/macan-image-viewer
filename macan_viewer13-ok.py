# macan_viewer_v12_final_panfix.py
...
class ImageViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        # --- PENAMBAHAN: Atribut untuk Pan/Geser Gambar ---
    ...
    def show_about_dialog(self):
        ...
        msg_box.setText("<b>Macan Image Viewer v2.3.0</b><br><br>" 
    ...
    # --- PENAMBAHAN: Helper function untuk memeriksa apakah gambar bisa di-pan ---
    ...
    def image_mouse_press(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            # --- MODIFIKASI: Logika untuk memilih antara Pan atau Crop ---
            if self.is_pannable():
                self.is_panning = True
                self.pan_last_pos = event.pos()
                self.image_label.setCursor(Qt.CursorShape.ClosedHandCursor)
            else: # Mulai cropping jika tidak bisa di-pan
    ...
    def image_mouse_move(self, event):
        # --- MODIFIKASI: Menambahkan logika untuk Pan dan update kursor ---
        if self.is_panning:
        ...
        elif self.is_pannable(): # Update kursor menjadi tangan terbuka saat hover
            self.image_label.setCursor(Qt.CursorShape.OpenHandCursor)
        else:
            self.image_label.unsetCursor()
            
    def image_mouse_release(self, event):
        # --- MODIFIKASI: Menambahkan logika untuk menghentikan Pan ---
        if self.is_panning:
            self.is_panning = False
            self.image_label.setCursor(Qt.CursorShape.OpenHandCursor) # Kembali ke tangan terbuka
        elif self.is_cropping:
            self.image_label.unsetCursor()
    ...
    def stop_slideshow(self):
        ...
        # Menggunakan try-except untuk menghindari error jika koneksi sudah terputus
        try:
            self.slideshow_timer.timeout.disconnect(self.advance_slideshow)
        except TypeError:
            pass # Abaikan error jika slot tidak terhubung
        self.slideshow_running = False
        self.slideshow_action.setIcon(self._create_svg_icon(self.play_svg))
