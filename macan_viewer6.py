...
class ImageViewer(QMainWindow):
    ...
    Perubahan: Toolbar putih, Fit-to-window, Resize & Drag, Nama file di status bar.
    """
    def __init__(self):
        super().__init__()
    ...
    def init_ui(self):
        ...
        self.setStyleSheet("""
            ...
            /* --- PERUBAHAN --- */
            QToolBar {
                background-color: #ffffff; /* Warna toolbar menjadi putih */
                border: none;
                padding: 5px;
            }
            ...
            QToolBar QToolButton:hover {
                background-color: #e0e0e0; /* Warna hover abu-abu muda */
                border-radius: 3px;
            }
            ...
            /* --- AKHIR PERUBAHAN --- */
            QStatusBar {
                background-color: #202020; /* Warna status bar sedikit digelapkan */
                color: #f0f0f0;
            ...
            /* --- PERUBAHAN --- */
            QStatusBar QLabel { /* Pastikan teks di status bar tetap putih */
                color: #f0f0f0;
            }
            QPushButton {
        ...
        self.image_label.setMouseTracking(True) # Aktifkan mouse tracking untuk label gambar juga
        ...
        # Widget utama butuh layout agar bisa menampung image_label
        main_widget = QWidget()
    ...
    def _create_actions(self):
        ...
        self.reset_zoom_action = QAction("Fit to Window", self) # Teks diubah
    ...
    def _create_tool_bar(self):
        ...
        # --- PERUBAHAN: Warna ikon diubah menjadi hitam (#333333) ---
        minimize_svg = """<svg viewBox="0 0 24 24"><path fill="none" stroke="#333333" stroke-width="2" d="M4 12 L20 12"></path></svg>"""
        maximize_svg = """<svg viewBox="0 0 24 24"><path fill="none" stroke="#333333" stroke-width="2" d="M4 4 L20 4 L20 20 L4 20 Z"></path></svg>"""
        self.restore_svg = """<svg viewBox="0 0 24 24"><path fill="none" stroke="#333333" stroke-width="2" d="M9 9 L20 9 L20 20 L9 20 Z M4 4 L15 4 L15 15 L4 15 Z"></path></svg>"""
        close_svg = """<svg viewBox="0 0 24 24"><path fill="none" stroke="#333333" stroke-width="2" d="M6 6 L18 18 M18 6 L6 18"></path></svg>"""
    ...
    def _create_status_bar(self):
        ...
        # --- PERUBAHAN: Menambahkan label untuk nama file ---
        self.filename_label = QLabel()
        self.statusbar.addWidget(self.filename_label, 1) # Stretch factor 1 agar mengisi ruang
    ...
    def open_image(self, file_path=None):
        ...
        if file_path:
            ...
            # --- PERUBAHAN: Setel mode fit-to-window saat gambar baru dibuka ---
    ...
    def _display_image(self):
        ...
        if self.fit_to_window:
            ...
            if w_orig > scroll_area_size.width() or h_orig > scroll_area_size.height():
                ...
                # Jika gambar sudah lebih kecil, tampilkan ukuran aslinya
                scaled_pixmap = pixmap
            # Hitung zoom factor berdasarkan hasil scaling
            if w_orig > 0: self.zoom_factor = scaled_pixmap.width() / w_orig
        else:
            # Logika zoom manual yang sudah ada
        ...
        self._update_status_bar() # Panggil update status bar setelah zoom dihitung

    def _update_status_bar(self):
        if self.cv_image is not None and self.image_path and os.path.exists(self.image_path):
            h, w, _ = self.cv_image.shape
            # --- PERUBAHAN: Tampilkan nama file di status bar ---
    ...
    def zoom_in(self):
        ...
        self.fit_to_window = False # Matikan mode fit-to-window
    ...
    def reset_zoom(self):
        if self.display_image is None: return
        self.fit_to_window = True # Aktifkan kembali mode fit-to-window
        self._display_image()

    def rotate_left(self):
        if self.display_image is not None:
            self.display_image = cv2.rotate(self.display_image, cv2.ROTATE_90_COUNTERCLOCKWISE)
            self.fit_to_window = True # Sesuaikan lagi ke jendela setelah rotasi
            self._display_image()
    ...
    def flip_image(self, flip_code):
        if self.display_image is not None:
            ...
            self._display_image() # Flip tidak mengubah dimensi, jadi tidak perlu fit-to-window
    ...
    # --- PERUBAHAN: Logika baru untuk Drag dan Resize Jendela ---
    def get_edge(self, pos):
        """Mendapatkan tepi jendela berdasarkan posisi kursor."""
        rect = self.rect()
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if self.resize_edge:
                ...
                event.accept()
            # Cek untuk drag (hanya di area toolbar)
    ...
    def image_mouse_release(self, event):
        if self.is_cropping:
            ...
            # Perhitungan offset jika gambar lebih kecil dari label
    ...
    def perform_crop(self):
        ...
        try:
            # Pastikan area crop berada dalam batas gambar
            ...
            self.fit_to_window = True # Sesuaikan gambar hasil crop ke jendela
            self._display_image()
