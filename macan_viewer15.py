# macan_viewer_v15_context_menu.py
...
class ResizeDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.percentage_widget.setLayout(percentage_layout)
    ...
    def on_mode_change(self, checked):
        ...
        self.width_edit.parent().setEnabled(is_pixels_mode)
        self.lock_button.setEnabled(is_pixels_mode)
        self.height_edit.parent().setEnabled(is_pixels_mode)
        self.percentage_widget.setVisible(not is_pixels_mode)
        if not is_pixels_mode:
            self.percentage_slider.setValue(100) # Reset on switch
...
class ImageViewer(QMainWindow):
    ...
    def open_image(self, file_path=None):
        ...
        if not file_path:
            self.stop_slideshow()
            file_path, _ = QFileDialog.getOpenFileName(self, "Open Image", "", "Image Files (*.png *.jpg *.jpeg *.bmp *.webp);;All Files (*)")
    ...
    def show_about_dialog(self):
        ...
        msg_box.setText("<b>Macan Image Viewer v2.5.0</b><br><br>" 
        ...
        msg_box.setStyleSheet("""
            QMessageBox { background-color: #f0f0f0; color: #1e1e1e; }
            QLabel { color: #1e1e1e; }
        """)
        msg_box.exec()
    ...
    def resize_image(self):
        ...
        if dialog.exec():
            ...
            try:
                ...
                save_path, _ = QFileDialog.getSaveFileName(self, "Save Resized Image As...", default_filename, filter_str)
    ...
    def save_image_as(self):
        if self.display_image is None: return
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Image As...", os.path.basename(self.image_path) if self.image_path else "","PNG (*.png);;JPG (*.jpg);;BMP (*.bmp)")
        if file_path:
            try:
