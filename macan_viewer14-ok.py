...
class ImageViewer(QMainWindow):
    ...
    Fix "Open in Explorer", update teks "About", Drag-and-Drop, Icon, Image Converter,
    Window Drag Fix, Rounded Corners, Zoom Slider, Arrow Key Navigation, Slideshow, Pan Zoomed Image,
    Crop Mode Button.
    """
    def __init__(self):
        super().__init__()
        ...
        # --- PENAMBAHAN: Atribut untuk Mode Crop ---
    ...
    def init_ui(self):
        ...
        self.setStyleSheet("""
            ...
            /* --- PENAMBAHAN: Styling untuk tombol Crop saat aktif --- */
            QToolBar QToolButton:checked {
                background-color: #4C566A; /* Warna latar belakang saat tombol ditekan/aktif */
    ...
    def _create_actions(self):
        ...
        # --- PENAMBAHAN: Aksi untuk Mode Crop ---
        self.crop_action = QAction("Crop Image", self)
        self.crop_action.setCheckable(True) # Membuat tombol bisa di-toggle
    ...
    def _create_tool_bar(self):
        ...
        # --- PENAMBAHAN: SVG Icon untuk Crop ---
        ...
        # --- PENAMBAHAN: Menambahkan tombol Crop ke toolbar ---
    ...
    def open_image(self, file_path=None):
        ...
        if file_path:
            # --- PENAMBAHAN: Keluar dari mode crop saat membuka gambar baru ---
    ...
    def show_about_dialog(self):
        ...
        # --- PENAMBAHAN: Memperbarui versi ---
        msg_box.setText("<b>Macan Image Viewer v2.4.0</b><br><br>" 
    ...
    def cancel_crop(self):
        ...
        # --- PENAMBAHAN: Keluar dari mode crop saat dibatalkan ---
        if self.is_in_crop_mode:
            self.crop_action.setChecked(False)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_Escape:
            # --- MODIFIKASI: Escape akan membatalkan crop atau keluar dari mode crop ---
            if self.is_cropping:
                self.cancel_crop()
            elif self.is_in_crop_mode:
                self.crop_action.setChecked(False) # Menonaktifkan tombol crop
        else:
            super().keyPressEvent(event)

    def is_pannable(self):
        """Memeriksa apakah gambar lebih besar dari area scroll, sehingga bisa di-pan."""
    ...
    def image_mouse_press(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            # --- MODIFIKASI: Logika baru yang memprioritaskan Mode Crop ---
            if self.is_in_crop_mode:
    ...
    def image_mouse_move(self, event):
        ...
        # --- MODIFIKASI: Logika kursor yang memprioritaskan mode crop ---
        elif self.is_in_crop_mode:
    ...
    def image_mouse_release(self, event):
        if self.is_panning:
            ...
            # Tidak lagi unset cursor di sini, biarkan crosshair tetap ada selama mode crop
            if self.origin_point == event.position().toPoint():
                self.cancel_crop()
                return

            self.is_cropping = False # Selesai menggambar, bukan selesai cropping
    ...
    def perform_crop(self):
        ...
        try:
            ...
            self.rubber_band.hide()
            self.crop_button.hide()
            # --- PENAMBAHAN: Otomatis keluar dari mode crop setelah berhasil ---
            self.crop_action.setChecked(False)
            
    # --- PENAMBAHAN: Metode baru untuk mengelola Mode Crop ---
    def toggle_crop_mode(self, checked):
        ...
        if checked:
            ...
            self.cancel_crop() # Membersihkan sisa seleksi jika ada

    def toggle_slideshow(self):
        """Memulai atau menghentikan slideshow."""
        if self.slideshow_running:
            self.stop_slideshow()
        else:
    ...
    def start_slideshow(self):
        ...
        self.slideshow_timer.start(3000) # Interval 3 detik
        self._update_action_states(True)

    def stop_slideshow(self):
        """Menghentikan timer slideshow dan mengembalikan UI ke normal."""
    ...
    def advance_slideshow(self):
        """Pindah ke gambar berikutnya dalam mode slideshow (dengan looping)."""
    ...
    def closeEvent(self, event):
        # --- FIX: Memperbaiki logika penutupan ---
        self.stop_slideshow()
        if self.converter_widget and self.converter_widget.isVisible():
            self.converter_widget.close()
        
        # Kode yang menyebabkan error telah dihapus, karena penanganan thread
        # sudah dilakukan di dalam `closeEvent` milik ImageConverterWidget.
        event.accept()
