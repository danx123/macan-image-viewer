# macan_viewer19.py (Dengan Integrasi Preset dan Drag & Drop)
...
class ManagePresetsDialog(QDialog):
    def __init__(self, preset_manager, parent=None):
        ...
        layout.addWidget(QLabel("Drag & drop .xmp files here or use buttons below."))
    ...
    def add_preset(self):
        file_paths, _ = QFileDialog.getOpenFileNames(self, "Select .xmp Preset File(s)", "", "XMP Files (*.xmp)")
        if file_paths:
    ...
    # --- PENAMBAHAN: Event handler untuk drag enter ---
    def dragEnterEvent(self, event):
        # Cek apakah data yang di-drag berisi URL (file)
        if event.mimeData().hasUrls():
            # Cek apakah ada setidaknya satu file .xmp
            if any(url.toLocalFile().lower().endswith('.xmp') for url in event.mimeData().urls()):
                # Terima aksi drag jika ada file .xmp
                event.acceptProposedAction()

    # --- PENAMBAHAN: Event handler untuk drop ---
    def dropEvent(self, event):
        urls = event.mimeData().urls()
        # Filter hanya file .xmp dari semua file yang di-drop
        xmp_files = [url.toLocalFile() for url in urls if url.toLocalFile().lower().endswith('.xmp')]

        if xmp_files:
            self._process_files(xmp_files)
        
        event.acceptProposedAction()

    # --- PENAMBAHAN: Fungsi terpusat untuk memproses file (dari dialog atau D&D) ---
    def _process_files(self, file_paths):
        ...
        # Siapkan pesan laporan untuk user
        message_parts = []
        if added_count > 0:
            message_parts.append(f"Successfully added {added_count} new preset(s).")
        if skipped_files:
            message_parts.append(f"\nSkipped {len(skipped_files)} preset(s) that already exist:\n- " + "\n- ".join(skipped_files))
        if error_files:
             message_parts.append(f"\nFailed to add {len(error_files)} preset(s) due to errors:\n- " + "\n- ".join(error_files))
...
class ImageViewer(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        msg_box.setText("<b>Macan Image Viewer v4.0.0</b><br><br>" 
    ...
    def _apply_preset(self, settings):
        ...
        try:
            ...
            # Rollback on error by popping from the undo stack
            if self.undo_stack:
    ...
    def _update_status_bar(self):
        if self.file_path and os.path.exists(self.file_path):
            ...
            if self.current_media_type == 'image' and self.display_image is not None:
                h, w, _ = self.display_image.shape
    ...
    def resizeEvent(self, event):
        ...
        if self.current_media_type == 'image' and self.display_image is not None and self.fit_to_window:
             self._display_image()
