# macan_viewer_v11_final_slideshow.py
...
class ImageViewer(QMainWindow):
    ...
    Window Drag Fix, Rounded Corners, Zoom Slider, Arrow Key Navigation, Slideshow.
    """
    def __init__(self):
        super().__init__()
        ...
        # --- PENAMBAHAN: Atribut untuk Slideshow ---
    ...
    def _create_actions(self):
        ...
        # --- PENAMBAHAN: Aksi untuk Slideshow ---
    ...
    def _create_tool_bar(self):
        ...
        # --- PENAMBAHAN: SVG Icons untuk Slideshow ---
        ...
        # --- PENAMBAHAN: Set icon awal slideshow ---
        ...
        # --- PENAMBAHAN: Menambahkan tombol slideshow ke toolbar ---
    ...
    def _update_action_states(self, enabled):
        # --- MODIFIKASI: Logika untuk menonaktifkan tombol saat slideshow berjalan ---
        if self.slideshow_running:
            actions_to_disable = [
                ...
                self.next_action, self.undo_action, self.redo_action
            ]
            for action in actions_to_disable:
                action.setEnabled(False)
            self.zoom_slider.setEnabled(False)
            self.slideshow_action.setEnabled(True) # Pastikan tombol slideshow tetap aktif untuk berhenti
            return
        ...
        # --- PENAMBAHAN: Mengaktifkan/menonaktifkan tombol slideshow berdasarkan jumlah gambar ---
        self.slideshow_action.setEnabled(enabled and len(self.current_folder_images) > 1)
        
    def open_image(self, file_path=None):
        if not file_path:
            # --- PENAMBAHAN: Hentikan slideshow saat membuka file baru ---
            self.stop_slideshow()
    ...
    def show_about_dialog(self):
        ...
        msg_box.setText("<b>Macan Image Viewer v2.2.0</b><br><br>" # Versi diperbarui
    ...
    def dropEvent(self, event):
        # --- PENAMBAHAN: Hentikan slideshow saat drop file baru ---
        self.stop_slideshow()
        urls = event.mimeData().urls()
        supported_extensions = ['.png', '.jpg', '.jpeg', '.bmp', '.webp']
        for url in urls:
    ...
    def image_mouse_press(self, event):
        ...
        # --- PENAMBAHAN: Hentikan slideshow saat memulai crop ---
        self.stop_slideshow()
    ...
    # --- PENAMBAHAN: Metode-metode baru untuk mengontrol slideshow ---
    ...
    def start_slideshow(self):
        ...
        self._update_action_states(True) # Perbarui status tombol
    ...
    def advance_slideshow(self):
        ...
        # Logika looping: jika di gambar terakhir, kembali ke awal
    ...
    def closeEvent(self, event):
        # --- PENAMBAHAN: Hentikan slideshow saat menutup aplikasi ---
        self.stop_slideshow()
        if self.converter_widget and self.converter_widget.isVisible():
            self.converter_widget.close()
        
        # Original close event from converter (Error jd dikomentari wwkwkwk)
        #if hasattr(self, 'thread') and self.thread and self.thread.isRuning():
            #self.worker.stop()
            #self.thread.quit()
            #self.thread.wait()
        #event.accept()
